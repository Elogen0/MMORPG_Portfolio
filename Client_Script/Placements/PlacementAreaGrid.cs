...
[RequireComponent(typeof(BoxCollider))]
	public class PlacementAreaGrid : MonoBehaviour, IPlacementArea
	{
		/// <summary>
		...
		public PlacementFitStatus Fits(IntVector2 gridPos, IntVector2 size)
		{
			...
			{
				return PlacementFitStatus.OutOfBounds;
			}
			...
			for (int y = gridPos.y; y < extents.y; y++)
			{
				for (int x = gridPos.x; x < extents.x; x++)
				{
					...
					{
						return PlacementFitStatus.Overlaps;
					}
				}
			}

			// If we've got this far, we've got a valid position.
			return PlacementFitStatus.Fits;
		}
